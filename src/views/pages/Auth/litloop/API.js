...
import addLogBase from 'views/pages/logs/addLogBase';
...
const LITLOOP_OAUTH_INSTANCE = axios.create({
  baseURL: 'http://localhost:8000/',
  timeout: 10000,
});
...
INSTANCE.interceptors.request.use(
  async (config) => {
    try {
      ...
      const session = await INSTANCE.post('currentSession');
...
const litloopAPI = {
  ...
  validateAccount: async (username) => {
    await INSTANCE.post(`/account/validate`, {})
  },
  ...
  updateTwitchUserData: async (data, access_token, refresh_token) =>
    await INSTANCE.put(`/users/twitch/update/`, {
      data,
      access_token,
      refresh_token,
    }).catch((e) => console.error(e)),
  ...
  getTwitchAccessToken: async (value) => {
    await INSTANCE.put(`/twitch/token`, {
      authCode: value,
    }).catch((e) => console.error(e))
  },
